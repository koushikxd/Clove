  ...
  useEffect(() => {
    if (!session.isPending && !session.data) {
      ...
      if (inviteToken) {
        ...
        if (inviteDetails?.inviteEmail) {
          loginUrl.searchParams.set("email", inviteDetails.inviteEmail)
        }
      }
      router.replace(`${loginUrl.pathname}${loginUrl.search}`)
    }
  }, [inviteDetails?.inviteEmail, inviteToken, router, session.data, session.isPending])
...
function CandidateOnboarding({
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (
      ...
      sessionEmail &&
      inviteDetails.inviteEmail.toLowerCase() !== sessionEmail.toLowerCase()
    ) {
      setError(`Use the invited email: ${inviteDetails.inviteEmail}`)
      return
    }

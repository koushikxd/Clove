...
export function SignUpPageClient() {
  ...
  const selectedRole =
    role === "candidate" && (candidateAllowed || candidatePending)
      ? "candidate"
      : "recruiter"
  const invitedEmail = invite?.inviteEmail ?? ""
  const isInviteCandidateFlow = selectedRole === "candidate" && candidateAllowed
  const effectiveEmail =
    isInviteCandidateFlow && invitedEmail ? invitedEmail : email
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (
      ...
      invitedEmail &&
      effectiveEmail.toLowerCase() !== invitedEmail.toLowerCase()
    ) {
      setLoading(false)
      setError(`Use the invited email: ${invitedEmail}`)
      return
    }
    ...
    <div className="flex min-h-svh items-center justify-center p-4">
      <Card className="w-full max-w-sm">
        <CardHeader>
          ...
          <CardDescription>
            {candidateAllowed
              ? "Create the invited candidate account with the invited email, or switch back to recruiter signup."
              : candidatePending
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="flex flex-col gap-4">
            ...
            {candidateAllowed ? (
              <p className="text-sm text-muted-foreground">
                Invite loaded for {invite?.job?.title ?? "this interview"} as{" "}
                {invite?.inviteEmail}.
              </p>
            ) : null}
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                disabled={isInviteCandidateFlow}
          ...
          <CardFooter className="mt-2 flex flex-col gap-3">
            ...
            <p className="text-center text-sm text-muted-foreground">
              ...
              <Link
                href={`/login?role=${selectedRole}${effectiveEmail ? `&email=${encodeURIComponent(effectiveEmail)}` : ""}${searchParams.get("invite") ? `&invite=${searchParams.get("invite")}` : ""}`}
